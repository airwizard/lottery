...
const { getPrizeConfig: localPrizeList, getCurrentPrize: currentPrize } =
...
function selectPrize(item: IPrizeConfig) {
  ...
  selectedPrize.value.separateCount = {
    enable: true,
    countList: [{ id: "0", count: item.count, isUsedCount: 0 }],
  };
}
...
function changePrizePerson(item: IPrizeConfig) {
  ...
  if (indexPrize > -1) {
    ...
    prizeList.value[indexPrize].isUsed
      ? (prizeList.value[indexPrize].isUsedCount =
          prizeList.value[indexPrize].count)
      : (prizeList.value[indexPrize].isUsedCount = 0);
  }
}
      ...
      <li
        ...
        :class="
          currentPrize.id === item.id
            ? 'border-1 border-dotted rounded-xl'
            : null
        "
      >
        <label class="max-w-xs mb-10 form-control">
          <div class="flex flex-col items-center gap-2 pt-5">
            <svg-icon
              ...
              :class="
                prizeList.indexOf(item) === 0 ? 'opacity-0 cursor-default' : ''
              "
              name="up"
              @click="sort(item, 1)"
            />
            <svg-icon
              ...
              name="down"
              :class="
                prizeList.indexOf(item) === prizeList.length - 1
        ...
        <label class="w-1/2 max-w-xs mb-10 form-control">
          ...
          <div
            ...
            :data-tip="`${t('table.isDone') + item.isUsedCount}/${item.count}`"
          >
            <progress
        ...
        >
          ...
          >
            ...
            >
              ...
              >
                <div
                  ...
                  :data-tip="`${t('tooltip.doneCount') + se.isUsedCount}/${
                    se.count
                  }`"
                >
                  <div
            ...
            <button v-else class="btn btn-secondary btn-xs">
              {{ t("button.setting") }}
            </button>
          </div>
        </label>
        <label class="w-full max-w-xs mb-10 form-control">
          ...
          <div class="flex gap-2">
            <button class="btn btn-error btn-sm" @click="delItem(item)">
              {{ t("button.delete") }}
            </button>
          </div>
        </label>
