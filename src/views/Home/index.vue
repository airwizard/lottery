  ...
  if (originPersonLength < totalCount) {
    ...
    for (let i = 0; i < repeatCount; i++) {
      tableData.value = tableData.value.concat(
        JSON.parse(JSON.stringify(originPersonData))
      );
    }
  } else {
...
async function setDefaultPersonList() {
  try {
    ...
    console.error("Failed to set default person list:", error);
    toast.open({
      message: t("error.failedToLoadDefaultData") || "加载默认数据失败",
      type: "error",
      position: "top-right",
      duration: 5000,
    });
  ...
  intervalTimer.value = setInterval(() => {
    ...
    for (let i = 0; i < indexLength; i++) {
      const randomCardIndex = Math.round(
        Math.random() * (tableData.value.length - 1)
      );
      const randomPersonIndex = Math.round(
        Math.random() * (allPersonList.value.length - 1)
      );
